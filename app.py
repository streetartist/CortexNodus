...
def run_training_thread(plan: Dict[str, Any]):
    global state
        ...
        with state_lock:
            ...
            if main_target_type == "Input":
                if val_metric < state.best_val_loss:
                    ...
                    best_path = os.path.join(os.getcwd(), "best_model.pt")
                    torch.save(model.state_dict(), best_path)
                    state.best_model_path = best_path
                    logger.info(f"💾 保存最佳模型 (验证损失: {val_metric:.4f})")
            elif val_metric > state.best_val_acc:
                state.best_val_acc = float(val_metric)
                best_path = os.path.join(os.getcwd(), "best_model.pt")
                torch.save(model.state_dict(), best_path)
                state.best_model_path = best_path
                logger.info(f"💾 保存最佳模型 (验证准确率: {val_metric:.4f})")
...
def run_training():
    try:
        ...
        t = threading.Thread(target=run_training_thread, args=(plan,), daemon=True)
        t.start()
        return jsonify({"ok": True})
...
def generate_script():
    try:
        graph = request.get_json(force=True)
        plan = parse_graph_to_plan(graph)
        script_path = os.path.join(os.getcwd(), "train_generated.py")
        ...
        with open(script_path, "w", encoding="utf-8") as f:
            f.write(code)
        return jsonify({"ok": True, "path": script_path})
    except Exception as e:
