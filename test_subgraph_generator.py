...
def test_subgraph_example():
    print("Testing Subgraph Example...")
    with open('example/subgraph_example.json', 'r') as f:
        graph = json.load(f)
    ...
    try:
        ...
        with open('generated_subgraph_script.py', 'w') as f:
            f.write(script)
            
        print("Script saved to generated_subgraph_script.py")
        
        # Try to compile/exec the script to check for syntax errors
        try:
            compile(script, 'generated_subgraph_script.py', 'exec')
